...
<div wire:poll.500ms id="game-container" data-strikes="{{ $gameSession ? $gameSession->strikes : 0 }}"
    ...
    {{-- OVERLAY TETOT (STRIKE < 3) --}} @if ($gameSession && $gameSession->strikes > 0 && !$isStrikeOut)
        {{-- FIX: Syntax wire:key sudah diperbaiki (tidak ada '}}' ganda) --}}
        <div wire:key="strike-overlay-{{ $gameSession->id }}-{{ $gameSession->strikes }}"
                ...
                @for ($i = 0; $i < $gameSession->strikes; $i++)
                    <div class="relative w-32 h-32 md:w-64 md:h-64 animate-bounce-in"
                        ...
                        <svg viewBox="0 0 100 100" class="w-full h-full drop-shadow-[0_0_50px_rgba(220,38,38,1)]">
                            <path d="M 20 20 L 80 80 M 80 20 L 20 80" stroke="#ef4444" stroke-width="12"
    ...
    @endif
        ...
        @if ($gameSession)
            <div class="absolute top-4 right-4 md:top-8 md:right-8 z-40 flex gap-2 md:gap-3">
                @for ($i = 1; $i <= 3; $i++)
                    <div
                        class="relative w-8 h-8 md:w-12 md:h-12 rounded-full border-2 flex items-center justify-center transition-all duration-500
