    <div wire:poll.500ms class="h-screen w-screen bg-slate-900 overflow-hidden relative font-sans select-none">
        {{-- ================= AUDIO UNLOCK (ANTI LIVEWIRE POLL) ================= --}}
        <div wire:ignore>
            <div id="audio-unlock-overlay"
                ...
                <div>
                    <h1 class="text-4xl font-black mb-4 animate-pulse">🔊 AKTIFKAN AUDIO</h1>
                    <p class="text-lg opacity-80">
                        Klik di mana saja atau tekan tombol<br>
                        untuk mengaktifkan sound effect
                    </p>
                </div>
            </div>
            ...
            <script>
                (function() {
                    ...
                    let audioUnlocked = localStorage.getItem('audio_unlocked') === 'true';
                    ...
                    async function unlockAudio() {
                        ...
                        try {
                            ...
                            console.log('🔓 Audio unlocked (persistent)');
                        } catch (e) {
                            console.warn('Audio unlock failed:', e);
                        }
                    }

                    // INIT (hanya sekali, polling tidak ngaruh)
                    if (!audioUnlocked) {
                        showOverlay();
                    }
                    ...
                    // ===== GLOBAL API (dipanggil operator / livewire) =====
                    window.playCorrect = () => {
                        if (!audioUnlocked) return;
                        correct.currentTime = 0;
                        correct.play();
            ...
            <div
                ...
                <div class="relative w-48 h-48 md:w-64 md:h-64 mb-8 animate-pulse">
                    <svg viewBox="0 0 100 100" class="w-full h-full drop-shadow-[0_0_50px_rgba(0,0,0,0.8)]">
                        <path d="M 20 20 L 80 80 M 80 20 L 20 80" stroke="#ef4444" stroke-width="15"
                            stroke-linecap="round" stroke-linejoin="round" />
                    </svg>
                </div>
                <h1
                    class="text-5xl md:text-7xl font-black text-white uppercase tracking-widest drop-shadow-lg text-center">
                    LOCKED
                </h1>
                <p
                ...
                <div class="flex gap-4 md:gap-12">
                    @for ($i = 0; $i < $gameSession->strikes; $i++)
                        <div class="relative w-32 h-32 md:w-64 md:h-64 animate-bounce-in"
                            ...
                            <div class="absolute inset-0 bg-red-600 rounded-full opacity-30 blur-2xl animate-pulse">
                            </div>
                ...
                @for ($i = 1; $i <= 3; $i++)
                    <div
                        ...
                        @if ($i <= $gameSession->strikes)
                            <svg class="w-5 h-5 md:w-7 md:h-7 text-red-500 drop-shadow-md" fill="none"
                                ...
                                @if ($answer->is_revealed)
                                    <div
                                        ...
                                        <div class="flex-grow flex items-center pr-4">
                                            <span
                                                class="font-black text-slate-900 uppercase leading-none tracking-tight"
            ...
            @if ($question)
                <div class="flex justify-center items-center pt-2 pb-2">
                    <div
                        ...
                        <div class="absolute top-0 left-0 w-full h-1/2 bg-gradient-to-b from-white/10 to-transparent">
                        </div>
