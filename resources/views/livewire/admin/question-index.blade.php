                    ...
                    <div
                        ...
                        <div class="bg-indigo-50/50 px-8 py-4 border-b border-indigo-100 flex items-center gap-2">
                            ...
                            <h3 class="font-bold text-indigo-900">
                                {{ $question_id ? 'Edit Pertanyaan' : 'Buat Pertanyaan Baru' }}</h3>
                        </div>
                        ...
                        <div class="p-6 md:p-8">
                            <form wire:submit.prevent="save">
                                <div class="mb-8">
                                    ...
                                    @error('question_text')
                                        <span class="text-red-500 text-xs mt-1 font-bold ml-1">{{ $message }}</span>
                                    @enderror
                                </div>
                                ...
                                <div class="mb-8">
                                    <div class="flex justify-between items-center mb-3 px-1">
                                        ...
                                        <div class="flex gap-2">
                                            <span
                                                class="text-[10px] bg-gray-100 text-gray-500 px-2 py-1 rounded font-mono">Total:
                                                {{ count($answers) }}</span>
                                            <span
                                                class="text-[10px] bg-yellow-100 text-yellow-700 px-2 py-1 rounded font-mono">Auto-Rank</span>
                                        </div>
                                    </div>
                                    ...
                                    <div class="bg-gray-50 p-6 rounded-2xl border border-gray-200 space-y-3">
                                        @foreach ($answers as $index => $answer)
                                            <div class="flex gap-3 items-center group animate-fade-in-down"
                                                ...
                                                <div
                                                    class="w-8 h-8 rounded-lg bg-white border border-gray-300 flex items-center justify-center text-xs font-bold text-gray-400 shadow-sm shrink-0">
                                                    {{ $index + 1 }}
                                                </div>
                                                ...
                                                <div class="w-24 relative">
                                                    <input type="number"
                                                        wire:model="answers.{{ $index }}.point" placeholder="0"
                                                ...
                                                @if (count($answers) > 1)
                                                    <button type="button"
                                                        ...
                                                        class="w-8 h-8 flex items-center justify-center rounded-lg bg-white border border-red-200 text-red-400 hover:text-white hover:bg-red-500 transition shadow-sm shrink-0"
                                        ...
                                        <div class="pt-2">
                                            <button type="button" wire:click="addAnswer"
                                                class="text-sm font-bold text-indigo-600 hover:text-indigo-800 hover:bg-indigo-50 px-4 py-2 rounded-lg transition flex items-center gap-2 border border-dashed border-indigo-300 w-full justify-center">
                                                <span>➕</span> Tambah Baris Jawaban
                                            </button>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <tr class="hover:bg-indigo-50/20 transition-colors group">
                                        ...
                                        <td class="px-6 py-4">
                                            ...
                                            <div class="text-[10px] text-gray-400 mt-1 flex items-center gap-2">
                                                <span>ID:
                                                    #{{ $q->id }}</span><span>•</span><span>{{ $q->created_at->diffForHumans() }}</span>
                                            </div>
                                        </td>
                                        <td class="px-6 py-4 text-center">
                                            <span
                                                class="inline-flex items-center px-2.5 py-1 rounded-md text-xs font-bold bg-gray-100 text-gray-600 border border-gray-200">{{ $q->answers_count }}</span>
                                        </td>
                                        <td class="px-6 py-4 text-center">
                                            @if ($q->is_active)
                                                <div
                                            ...
                                            @else
                                                <button wire:click="toggleActive({{ $q->id }})"
                                                    class="text-gray-400 hover:text-indigo-600 font-bold text-[10px] uppercase border border-gray-200 hover:border-indigo-300 px-3 py-1 rounded-full transition bg-white hover:bg-indigo-50">Set
                                                    Aktif</button>
                                            @endif
                                        </td>
                                    ...
                                    <tr>
                                        <td colspan="5" class="px-6 py-16 text-center">
                                            <div class="flex flex-col items-center justify-center opacity-50">
                                                <div class="bg-gray-100 p-4 rounded-full mb-3"><span
                                                        class="text-3xl">📭</span></div>
                                                <p class="text-gray-600 font-bold">Belum ada soal tersedia</p>
                                                <p class="text-sm text-gray-400 mt-1">Tekan tombol "Buat Soal Baru"
                                                    untuk memulai</p>
                                            </div>
                                        </td>
                                    </tr>
